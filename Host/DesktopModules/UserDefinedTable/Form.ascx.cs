...
namespace DotNetNuke.Modules.UserDefinedTable
{
    public partial class EditForm : PortalModuleBase, IActionable, IFormEvents
    {
        ...
        protected SqlDataProvider Provider
        {
            get
            {
                if (_provider == null)
        ...
        /// <summary>
        /// Updates list of HRs in dropdown list of HRs for list of vacancies, made via FormAndList module
        /// and set default HR for all vacancies, that reference inactive HR.
        /// </summary>
        private void UpdateActiveHRsInVacancies()
        {
            Provider.UpdateActiveHRsInVacancies();
        }
        private void UpdateActiveHRsInCamps()
        {
            Provider.UpdateActiveHRsInCamps();
        }
        ...
        #region Js Crutch members
        private string[] JsCrutchSettingsKeys { get; } = new string[] {
                    SettingName.JsCrutchMinWidth,
                    SettingName.JsCrutchMinHeight,
                    SettingName.JsCrutchMaxWidth,
                    SettingName.JsCrutchMaxHeight
                };

        public bool NeedToApplyJsCrutch
        {
            get
            {
                return !string.IsNullOrEmpty(JsCrutchBackgroundImgFolder) &&
                    JsCrutchSettingsKeys.Any(key => base.Settings.Contains(key) &&
                            !string.IsNullOrEmpty((string)base.Settings[key]) &&
                            ((string)base.Settings[key]) != "0");
            }
        }

        public string JsCrutchBackgroundImgFolder
        {
            get
            {
                if (!base.Settings.Contains(SettingName.JsCrutchBackgroundImgFolderId))
                {
                    return "";
                }

                string folderId = (string)base.Settings[SettingName.JsCrutchBackgroundImgFolderId];
                return Provider.GetFolderPath(int.Parse(folderId));
            }
        }
        public string JsCrutchMinWidth => base.Settings.Contains(SettingName.JsCrutchMinWidth) ?
            (string)base.Settings[SettingName.JsCrutchMinWidth] : "0";
        public string JsCrutchMinHeight => base.Settings.Contains(SettingName.JsCrutchMinHeight) ?
            (string)base.Settings[SettingName.JsCrutchMinHeight] : "0";
        public string JsCrutchMaxWidth => base.Settings.Contains(SettingName.JsCrutchMaxWidth) ?
            (string)base.Settings[SettingName.JsCrutchMaxWidth] : "0";
        public string JsCrutchMaxHeight => base.Settings.Contains(SettingName.JsCrutchMaxHeight) ?
            (string)base.Settings[SettingName.JsCrutchMaxHeight] : "0";
        #endregion
    }
}